...
/// 用户在 ChipBar 上点击追问 → 用这个 messages 让 LLM 流式回答。
///
/// `unit_explanation`：当前单元已经讲过的全部讲解（前端从 teach_pack 拿），
///                     用来限定回答上下文不跑题；
/// `prev_followups`  ：本单元已经发起的历史追问（按时间顺序），让 LLM 看见上下文连续性。
pub fn build_unit_followup_messages(
    ...
    let system = r#"你是一名一对一学习教练，学生刚学完一个知识单元后向你追问。
回答风格：
- 极简、直接、像朋友讲解，不要客套，不要"好问题！"
- 紧扣**本单元已讲过的内容**作答；超出本单元范围时，明确指出"这点超出本单元，简要补一句" 然后给 1-2 句话
- 用流畅 markdown 自然展开，避免大段公式式列点
- 关键术语用 **加粗**；要点用 1./2./3.
- **严禁**使用 ## / ### 标题（前端把追问回答渲染在小卡里，没必要有 H 标题）
- 长度 80-300 字，看问题复杂度调
- 如有必要可以引用讲解里的具体段落，但**不要复读整段讲解**"#;
...
请回答。"#,
        unit_explanation = unit_explanation.chars().take(3000).collect::<String>(),
    );
