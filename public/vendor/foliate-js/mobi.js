...
// PATCH (doc-reader): MOBI 头里 `encoding` 字段在大量中文电子书里是错的——
// 常见 codepage=1252 但正文是 utf-8 / gb18030；甚至 codepage=65001 (utf-8)
// 但正文实际是 gb18030（出版社或转换工具瞎填）。
// 因此**所有** MOBI 都走自适应嗅探（见 makeAdaptiveDecoder），不再保留 fast-path。
// 此表仅保留为占位（如未来要硬编码白名单可用）。
const MOBI_ENCODING = {}
...
// PATCH (doc-reader): 自适应解码器（每段独立判定，不锁定）
//
// **关键事实**：z-library 等二次转换的中文 mobi 经常是**混合编码** ——
//   - 封面/版权页/目录这种 KF8 包装段是 UTF-8
//   - 真正的章节正文是 GB18030
//   - mobi.title 又可能是 UTF-8 或 GB18030
//
// 所以"锁定一次定终身"的策略必然失败：锁 utf-8 → 正文乱；锁 gb18030 → 元信息乱。
//
// 新策略：**每个 buffer 独立判定**
//   1) UTF-8 严格(fatal)解码：成功 → 字节流必然是合法 UTF-8 → 直接返回
//      （任何 GB18030/Big5 字节流几乎不可能在长度 ≥ 几十字节时凑巧通过 fatal-utf-8）
//   2) 失败 → 用 score(cjk-replacement-suspicious) 在 gb18030 / big5 中选最优
//   3) 短 buffer (< 8 字节) 走 utf-8 默认（ASCII 等价）
//
// 性能：每次 decode 多一次 O(n) 字节扫描（fatal 解码本身就是 O(n)），
// 3KB chunk < 1ms，整本书几十次 decode 总耗时 ms 级，可忽略。
const ADAPTIVE_CANDIDATES_FALLBACK = ['gb18030', 'big5']

const makeAdaptiveDecoder = (tag = 'unnamed') => {
    let calls = 0
    /** 累计每种编码出现次数，方便 console 摘要展示分布 */
    const stats = { 'utf-8': 0, 'gb18030': 0, 'big5': 0, 'ascii': 0 }
    // 每次 decode 都 new 新的 TextDecoder，避免 fatal 失败 / streaming 状态残留
    const decodeNonFatal = (enc, ab) => {
        try { return new TextDecoder(enc).decode(ab) } catch { return null }
    }
    const scoreString = (s) => {
        if (s == null) return -Infinity
        let cjk = 0, repl = 0, suspicious = 0
        for (let i = 0; i < s.length; i++) {
            const c = s.charCodeAt(i)
            if (c === 0xFFFD) repl++
            else if ((c >= 0x4E00 && c <= 0x9FFF) || (c >= 0x3400 && c <= 0x4DBF)) cjk++
            else if ((c >= 0xE000 && c <= 0xF8FF) || (c >= 0xF900 && c <= 0xFAFF)) suspicious++
            else if (c >= 0x80 && c < 0xA0) suspicious++
        }
        return cjk * 10 - repl * 100 - suspicious * 5
    }
    /**
     * 调试模式开关：默认关闭。需要诊断时在 Tauri 主窗口 Console 跑：
     *   localStorage.setItem('MOBI_DEBUG', '1')
     * 然后刷新即可看到 main-text decoder 的详细 log 与 utf-8/gb18030/big5 三路对比。
     */
    let MOBI_DEBUG = false
    try {
        MOBI_DEBUG = !!(globalThis?.localStorage?.getItem('MOBI_DEBUG'))
    } catch { /* worker / SSR 无 localStorage */ }
    const VERBOSE_MAIN = MOBI_DEBUG && tag === 'main-text'
    const logDist = (chosen, len) => {
        if (!MOBI_DEBUG) return
        try {
            if (VERBOSE_MAIN) {
                console.log(`[mobi:${tag}] decode #${calls} → ${chosen} (len=${len}), dist=${JSON.stringify(stats)}`)
                return
            }
            if (calls <= 5 || calls % 50 === 0) {
                console.log(`[mobi:${tag}] dist after ${calls} calls:`, JSON.stringify(stats))
            }
        } catch { /* noop */ }
    }
    return {
        decode(buffer, options) {
            calls++
            const ab = buffer instanceof Uint8Array ? buffer : new Uint8Array(buffer)

            // 短 buffer / 全 ASCII：用 utf-8 默认（ASCII 等价）
            if (ab.length < 8) {
                stats.ascii++
                logDist('ascii', ab.length)
                return new TextDecoder('utf-8').decode(buffer, options)
            }
            let hasNonAscii = false
            for (let i = 0; i < ab.length; i++) {
                if (ab[i] >= 0x80) { hasNonAscii = true; break }
            }
            if (!hasNonAscii) {
                stats.ascii++
                logDist('ascii', ab.length)
            ...
            // 优先 UTF-8 严格：成功 = 字节流必然合法 UTF-8
            try {
                const out = new TextDecoder('utf-8', { fatal: true }).decode(ab)
                stats['utf-8']++
                logDist('utf-8-strict', ab.length)
                // 调试模式：对照试解 GB18030 / Big5，对照打印 hex 头
                if (MOBI_DEBUG && VERBOSE_MAIN && ab.length > 1000 && calls <= 3) {
                    try {
                        const gbkStr = new TextDecoder('gb18030').decode(ab)
                        const big5Str = new TextDecoder('big5').decode(ab)
                        const hex = Array.from(ab.slice(0, 64))
                            .map(b => b.toString(16).padStart(2, '0')).join(' ')
                        console.log(
                            `[mobi:${tag}] decode #${calls} compare:\n`
                            + `  hex(64)  : ${hex}\n`
                            + `  utf-8    : ${out.slice(0, 120).replace(/\s+/g, ' ')}\n`
                            + `  gb18030  : ${gbkStr.slice(0, 120).replace(/\s+/g, ' ')}\n`
                            + `  big5     : ${big5Str.slice(0, 120).replace(/\s+/g, ' ')}`
                        )
                    } catch { /* noop */ }
                }
                return out
            ...
            // UTF-8 失败 → 在 gb18030 / big5 中评分选最优
            let best = 'gb18030', bestScore = -Infinity
            const scores = {}
            for (const enc of ADAPTIVE_CANDIDATES_FALLBACK) {
                const s = decodeNonFatal(enc, ab)
                const sc = scoreString(s)
                scores[enc] = sc
                if (sc > bestScore) { bestScore = sc; best = enc }
            }
            stats[best]++
            if (MOBI_DEBUG && (VERBOSE_MAIN || calls <= 3 || (ab.length >= 1024 && calls % 20 === 0))) {
                try {
                    console.log(`[mobi:${tag}] decode #${calls} utf-8 strict failed → ${best} (len=${ab.length}, scores=${JSON.stringify(scores)})`)
                } catch { /* noop */ }
            }
            logDist(best, ab.length)
            return new TextDecoder(best).decode(buffer, options)
        },
    }
}

const getDecoder = (x, tag = 'unknown') => {
    const known = MOBI_ENCODING[x]
    if (known) return new TextDecoder(known)
    // 任何非已知 codepage（含 65001 / 1252 / 0 等）都走自适应
    return makeAdaptiveDecoder(tag)
}
...
const getIndexData = async (indxIndex, loadRecord) => {
    ...
    const decoder = getDecoder(indx.encoding, 'indx')
...
const getEXTH = (buf, encoding) => {
    ...
    const decoder = getDecoder(encoding, 'exth')
    const results = {}
    let offset = 12
...
export class MOBI extends PDB {
    ...
    async #setup() {
        ...
        // PATCH (doc-reader): 标记此 decoder 为"正文 decoder"，便于 lock log 区分 EXTH/正文
        this.#decoder = getDecoder(mobi.encoding, 'main-text')
        try {
            if (globalThis?.localStorage?.getItem('MOBI_DEBUG')) {
                console.log(`[mobi] MOBI #setup: encoding=${mobi.encoding}, version=${mobi.version}, compression=${palmdoc.compression}`)
            }
        } catch { /* noop */ }
